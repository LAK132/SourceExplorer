...
namespace lak
{
    ...
    tinflate_error_t tinflate(const std::vector<uint8_t> &compressed,
        std::deque<uint8_t> &output, uint32_t *crc)
    {
        decompression_state_t state;

        state.state     = tinflate_state_t::INITIAL;
        state.crc       = 0;
        state.bitAccum  = 0;
        state.numBits   = 0;
        state.final     = 0;

        return tinflate(compressed, output, crc, state);
    }

    tinflate_error_t tinflate(const std::vector<uint8_t> &compressed,
        std::deque<uint8_t> &output, uint32_t *crc, decompression_state_t &state)
    {
        state.begin = compressed.cbegin();
        state.end = compressed.cend();

        if (state.state == tinflate_state_t::INITIAL ||
            state.state == tinflate_state_t::PARTIAL_ZLIB_HEADER)
        {
            uint16_t zlibHeader;
            if (compressed.size() == 0)
            {
                return tinflate_error_t::OUT_OF_DATA;
            }

            if (state.state == tinflate_state_t::INITIAL && compressed.size() == 1)
            {
            ...
            if (state.state == tinflate_state_t::PARTIAL_ZLIB_HEADER)
            {
            ...
            if ((zlibHeader & 0x8F00) == 0x0800 && (zlibHeader % 31) == 0)
            {
                if (zlibHeader & 0x0020)
                    return tinflate_error_t::CUSTOM_DICTIONARY;

                state.begin += (state.state == tinflate_state_t::PARTIAL_ZLIB_HEADER ? 1 : 2);
                if (state.begin >= state.end)
                    return tinflate_error_t::OUT_OF_DATA;
            }
            else if (state.state == tinflate_state_t::PARTIAL_ZLIB_HEADER)
            {
                state.bitAccum = state.firstByte;
                state.numBits = 8;
            }
            state.state = tinflate_state_t::HEADER;
        }

        do {
            tinflate_error_t res = tinflate_block(output, state);
            if (res != tinflate_error_t::OK)
                return res;
        } while (!state.final);
        ...
        return tinflate_error_t::OK;
    }

    tinflate_error_t tinflate_block(std::deque<uint8_t> &output, decompression_state_t &state)
    {
        ...
        switch (state.state)
        {
            #define JUMP_STATE(s) case tinflate_state_t::s: goto state_##s
            JUMP_STATE(HEADER);
            ...
            state_HEADER:
                ...
                if (state.blockType == 3)
                {
                    ...
                    return tinflate_error_t::INVALID_BLOCK_CODE;
                }

                if (state.blockType == 0)
                {
                    state.numBits = 0;
                    state.state = tinflate_state_t::UNCOMPRESSED_LEN;
            state_UNCOMPRESSED_LEN:
                    if (getBits(16, state.len)) goto out_of_data;
                    state.state = tinflate_state_t::UNCOMPRESSED_ILEN;
            state_UNCOMPRESSED_ILEN:
                    ...
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                        return tinflate_error_t::CORRUPT_STREAM;
                    }
                    state.nread = 0;
                    state.state = tinflate_state_t::UNCOMPRESSED_DATA;
            ...
            state_READ_CODE_LENGTHS:
                    ...
                    if (gen_huffman_table(19, state.codelenLen, false, state.codelenTable) != OK)
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                        return tinflate_error_t::HUFFMAN_TABLE_GEN_FAILED;
                    }

                    {
                        ...
                        state.lastValue = 0;
                        state.counter = 0;
                        state.state = tinflate_state_t::READ_LENGTHS;
            state_READ_LENGTHS:
                        repeatCount = 0;
                        while (state.counter < state.literalCount + state.distanceCount)
                        {
                            if (repeatCount == 0)
                            {
                                ...
                                {
                                    state.state = tinflate_state_t::READ_LENGTHS_16;
            state_READ_LENGTHS_16:
                                ...
                                {
                                    state.lastValue = 0;
                                    state.state = tinflate_state_t::READ_LENGTHS_17;
            state_READ_LENGTHS_17:
                                ...
                                {
                                    state.lastValue = 0;
                                    state.state = tinflate_state_t::READ_LENGTHS_18;
            state_READ_LENGTHS_18:
                ...
                {
                    uint32_t distance;
                    state.state = tinflate_state_t::READ_SYMBOL;
            state_READ_SYMBOL:
                    ...
                    {
                        state.state = tinflate_state_t::READ_LENGTH;
            state_READ_LENGTH:
                        {
                    ...
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                        return tinflate_error_t::INVALID_SYMBOL;
                    }

                    state.state = tinflate_state_t::READ_DISTANCE;
            state_READ_DISTANCE:
                    ...
                    {
                        state.state = tinflate_state_t::READ_DISTANCE_EXTRA;
            state_READ_DISTANCE_EXTRA:
                    ...
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                        return tinflate_error_t::INVALID_DISTANCE;
                    }
            ...
            default:
                return tinflate_error_t::INVALID_STATE;
        }
    ...
    tinflate_error_t gen_huffman_table(uint32_t symbols, const uint8_t *lengths,
        bool allowNoSymbols, int16_t *table)
    {
        ...
        if (totalCount == 0)
        {
            return allowNoSymbols ? OK : NO_SYMBOLS;
        }
        ...
        for (uint32_t i = 1; i < 16; ++i)
        {
            ...
            if (firstCode[i] + lengthCount[i] > (uint16_t)1 << i)
                return tinflate_error_t::TOO_MANY_SYMBOLS;
        }
        if (firstCode[15] + lengthCount[15] != (uint16_t)1 << 15)
            return tinflate_error_t::INCOMPLETE_TREE;
