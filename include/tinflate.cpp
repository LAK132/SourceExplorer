...
namespace lak
{
            ...
            state.state == state_t::PARTIAL_ZLIB_HEADER)
        {
            ...
            if ((zlibHeader & 0x8F00) == 0x0800 && (zlibHeader % 31) == 0)
            {
                if (zlibHeader & 0x0020)
                    return error_t::CUSTOM_DICTIONARY;
    ...
    )
    {
        ...
        auto push = [&output, &icrc](const uint8_t val)
        {
            ...
            icrc = crc32_table[(icrc & 0xFF) ^ val] ^
                ((icrc >> 8) & 0xFFFFFFUL);
        };
        ...
        switch (state.state)
        {
            case state_t::HEADER:
                ...
                if (state.blockType == 0)
                {
                    ...
                    state.state = state_t::UNCOMPRESSED_LEN; [[fallthrough]];
            case state_t::UNCOMPRESSED_LEN:

                    if (state.get_bits(16, state.len)) return out_of_data();

                    state.state = state_t::UNCOMPRESSED_ILEN; [[fallthrough]];
            case state_t::UNCOMPRESSED_ILEN:
                    ...
                    state.state = state_t::UNCOMPRESSED_DATA; [[fallthrough]];
            ...
            case state_t::CODELEN_COUNT:
                    ...
                    state.state = state_t::READ_CODE_LENGTHS; [[fallthrough]];
            case state_t::READ_CODE_LENGTHS:
                    ...
                    if (gen_huffman_table(19, state.codelenLen, false, state.codelenTable) != error_t::OK)
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                    ...
                    state.state = state_t::READ_LENGTHS; [[fallthrough]];
            case state_t::READ_LENGTHS:
                    ...
                    while (state.counter < state.literalCount + state.distanceCount)
                    {
                        if (state.repeatCount == 0)
                        {
                            ...
                            {
                                state.state = state_t::READ_LENGTHS_16; [[fallthrough]];
            case state_t::READ_LENGTHS_16:
                            ...
                            else if (state.symbol == 17)
                            {
                                ...
                                state.state = state_t::READ_LENGTHS_17; [[fallthrough]];
            case state_t::READ_LENGTHS_17:
                            ...
                            else if (state.symbol)
                            {
                                ...
                                state.state = state_t::READ_LENGTHS_18; [[fallthrough]];
            case state_t::READ_LENGTHS_18:
                    ...
                    if (gen_huffman_table(state.literalCount, state.literalLen,
                            false, state.literalTable) != error_t::OK ||
                        gen_huffman_table(state.distanceCount, state.distanceLen,
                            true, state.distanceTable) != error_t::OK)
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
                        return error_t::HUFFMAN_TABLE_GEN_FAILED;
                    }
                ...
                for (;;)
                {
                    state.state = state_t::READ_SYMBOL; [[fallthrough]];
            case state_t::READ_SYMBOL:
                    ...
                    {
                        state.state = state_t::READ_LENGTH; [[fallthrough]];
            case state_t::READ_LENGTH:
                    ...
                    state.state = state_t::READ_DISTANCE; [[fallthrough]];
            case state_t::READ_DISTANCE:
                    ...
                    {
                        state.state = state_t::READ_DISTANCE_EXTRA; [[fallthrough]];
            case state_t::READ_DISTANCE_EXTRA:
                        ...
                        if (state.get_bits(distanceBits, state.distance))
                            return out_of_data();
                        state.distance += 1 + ((2 + (state.symbol & 1)) << distanceBits);
                    }
                    else
                    {
                        state.crc = ~icrc & 0xFFFFFFFFUL;
