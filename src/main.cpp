...
void MenuBar(float FrameTime)
{
  if (ImGui::BeginMenu("File"))
  {
    ...
    SrcExp.exe.attempt |=
      ImGui::MenuItem(SrcExp.babyMode ? "Open And Dump..." : "Open...",
                      nullptr);
    SrcExp.sortedImages.attempt |=
      ImGui::MenuItem("Dump Sorted Images...", nullptr, false,
                      !SrcExp.babyMode);
    ...
    SrcExp.binaryFiles.attempt |=
      ImGui::MenuItem("Dump Binary Files...", nullptr, false,
                      !SrcExp.babyMode);
...
void Navigator()
{
  if (SrcExp.loaded)
  {
    if (SrcExp.state.game.title)
      ImGui::Text("Title: %s", lak::strconv<char>(
                    SrcExp.state.game.title->value).c_str());
    if (SrcExp.state.game.author)
      ImGui::Text("Author: %s", lak::strconv<char>(
                    SrcExp.state.game.author->value).c_str());
    if (SrcExp.state.game.copyright)
      ImGui::Text("Copyright: %s", lak::strconv<char>(
                    SrcExp.state.game.copyright->value).c_str());
    if (SrcExp.state.game.outputPath)
      ImGui::Text("Output: %s", lak::strconv<char>(
                    SrcExp.state.game.outputPath->value).c_str());
    if (SrcExp.state.game.projectPath)
      ImGui::Text("Project: %s", lak::strconv<char>(
                    SrcExp.state.game.projectPath->value).c_str());
...
void AudioExplorer(bool &Update)
{
  ...
  if (!playing && ImGui::Button("Play"))
  {
    ...
    if (audioDevice == 0)
      audioDevice = SDL_OpenAudioDevice(nullptr, false, &audioSpec,
                                        &audioSpecGot, 0);
...
void SourceExplorerMain(float FrameTime)
{
  ...
  else
  {
    ...
    ImGui::Text("To open a game either drag and drop it into this window or "
                "go to [File]->[Open]\n");
    ImGui::Text("If Source Explorer cannot open/throws errors while opening a "
...
{
  if (bytePairsMode) SourceBytePairsMain(FrameTime);
  else               SourceExplorerMain(FrameTime);
}
