  ...
  const char magic_char)
{
  ...
  for (int i = 0; i < 64; ++i)                         // eax@1
  {
    _mm_storeu_si128(
      bufferPtr,
      _mm_add_epi32(_mm_shuffle_epi32(_mm_cvtsi32_si128(i * 4), 0), offset));
    bufferPtr++;
  }
  ...
  {
    hash = (hash << 7) + (hash >> 1);
    if (v15) accum += ((hash & 1) | 2) * *key;

    if (hash == *key)
    {
      if (v15) rtn = (accum == *(key + 1));
  ...
  std::vector<uint8_t> &chunk, decode_buffer_t &decodeBuffer)
{
  ...
  {
    ++i;
    i2 += (uint8_t)decodeBuffer.m128i_i32[i];
  ...
  const char magic_char)
{
  ...
  if (magic_key.size() < 256)
    ERROR("Magic Key Too Small: 0x" << magic_key.size());
